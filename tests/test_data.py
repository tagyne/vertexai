...
def test_prepare_dataset_imputes_missing_categorical_values() -> None:
    frame = pd.DataFrame(
        {
            "student_id": [1], "gender": ["Female"], "study_time_hours": [4.0],
            "attendance_percent": [88.0], "sleep_hours": [7.0],
            "parental_education": [None], "internet_access": ["Yes"],
            "extracurricular_activities": ["Yes"], "part_time_job": ["No"],
            "previous_grade": [76.9], "final_exam_score": [76.4], "final_grade": ["A"],
        }
    )

    features, _ = prepare_dataset(frame)

    assert features.loc[0, "parental_education"] == "Unknown"
