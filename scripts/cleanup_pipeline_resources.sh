...
echo "- pipeline artifacts under gs://<ML bucket>/pipeline-root/ and pipeline-tmp/"
...
while read -r bucket; do
  ...
  --filter='labels.project=student-performance-mlops AND labels.managed_by=vertex-pipeline AND labels.environment=dev' \
  --format='value(name)')
echo "Labeled models and pipeline artifacts removed. Pipeline history remains in Vertex metadata. No Terraform-owned resource was targeted."
